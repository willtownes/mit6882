...
\end{matrix*}\right.
\end{align*}
or as in \cite{fox_bayesian_2009}, sample $I(t_{ji} = t_{new}|k_{jt_{new}} = k)$ directly from:
\begin{align*}
...
\begin{align*}
f_k^{-x_{ji}}(x_{ji}) & \sim 
N( \frac{n_{k}^{-(ji)}\sigma^2}{ n_{k}^{-(ji)}\sigma^2 + 1} 
\bar{\bx}_k^{-(ji)}, 
(1 + \frac{\sigma^2}{n_{k}^{-(ji)} \sigma^2 + 1} ) \bI)
\end{align*}

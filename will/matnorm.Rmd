...
bayes_mlinreg_post<-function(Y,X,M_A,M_B,K_A,K_B,nu0,Delta0){
  ...
  Syx1<-Y%*%t(X)+M_A%*%K_A
  Syx2<-Y+M_B%*%K_B
  Omega_a<-Saxx-X%*%solve(Sbxx,t(X))
  Omega_b<-Sbxx-t(X)%*%solve(Saxx,X)
  Sxx<-rbind(cbind(Saxx,X),cbind(t(X),Sbxx))
  OaiSyx1t<-solve(Omega_a,t(Syx1))
  SxxiSyxt<-rbind(OaiSyx1t-solve(Omega_a,X)%*%solve(Sbxx,t(Syx2)),
                  solve(Omega_b,t(Syx2))-solve(Sbxx,t(X))%*%OaiSyx1t)
  #M<-t(SxxiSyxt)
  #K<-Sxx
  Syy<-Y%*%t(Y)+M_A%*%K_A%*%t(M_A)+M_B%*%K_B%*%t(M_B)
  Sybarx<-Syy-cbind(Syx1,Syx2)%*%SxxiSyxt
  return(list(M=t(SxxiSyxt),K=Sxx,nun=nu0+N,Deltan=Delta0+Sybarx))
}

res<-bayes_mlinreg_post(Y,X,M_A,M_B,K_A,K_B,nu0,Delta0)
#posterior mean of covariance matrix Sigma
res$Deltan/(res$nun-d-1)
#posterior mean of A
res$M[,1:4]
#posterior mean of B
res$M[,5:20]

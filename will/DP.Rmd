...
```{r}
dp_gmm<-function(N,K,alpha,m,k,df,precision_matrix,verbose=FALSE){
...
vals<-dp_gmm(1000,100,1,c(5,-5),.1,3,matrix(c(1,.5,.5,1),nrow=2),verbose=TRUE)
...
$$\kappa_N = \kappa_0+N$$
$$\nu_N = \nu_0+N$$
$$S_N = S_0+S+\kappa_0 m_0 m_0' - \kappa_N m_N m_N'$$
...
rstick_post<-function(alpha,categ_counts){
  # returns a truncated stick breaking vector of weights

...
```{r}
init_mniw_hyper<-function(y,D=ncol(y),hyper=list()){
  ...
  Tmax<-nrow(y)
  time<-1:Tmax
  lowess_f<-5/Tmax #fraction of data points used to estimate x from y, essentially 5 nearest neighbors
...
for(j in 1:500){
  ...
  xhat<-rLDS(1,y,z,theta_hat,lambda_hat)[[1]]
  if(j%%25==0){

...
x<-xy[["x"]]
y<-xy[["y"]]
#colnames(x_full)<-c("pos_horiz","pos_vert","velo_horiz","velo_vert")
#colnames(y_full)<-c("pos_horiz","pos_vert")
...
theta[[2]]<-list(A=diag(4),B=c(20,20,0,0),Sigma=Sigma)
z<-rep(2,100)
xy<-gen_lds(z,x_full[iter_max,],theta,lambda)
x<-xy[["x"]]
y<-xy[["y"]]
...
```{r}
#dynamical parameters for linear motion
theta[[2]]<-list(A=diag(c(1,1,0,0)),B=c(200,50,0,0),Sigma=Sigma)
z2<-rep(2,100)
xy<-gen_lds(z2,x[iter_max,],theta,lambda) 
x<-rbind(x,xy[["x"]])
y<-rbind(y,xy[["y"]])
z<-c(z,z2)

...
The curve indicates the true (discretized) trajectory, the circles represent the noisy observations. The task now is to try to infer the curve given only the circles. We apply the backwards message passing, forwards sampling approach of Fox et al. The backwards message passing algorithm is Algorithm 19 in Fox's thesis.
...
# z is a vector of mode indicators
# pars is a list, one element for each unique mode
...
for(t in Tmax:1){
  ...
  B<-par_t[["B"]]
  Sigma<-par_t[["Sigma"]]
  Jt<-t(solve(Lambda+Sigma,Lambda))
  Lt<-eye - Jt
...
### Backward messages have all been computed
### Now forward sampling
xs<-matrix(NA,nrow=Tmax,ncol=ncol(Sigma))
for(t in 1:Tmax){
  par_t<-pars[[z[t]]]
  A<-par_t[["A"]]
  B<-par_t[["B"]]
  Sigma<-par_t[["Sigma"]]
  info_mat<-solve(solve(Sigma)+Lambda_tt[[t]])
  offset<-solve(Sigma,A%*%
  xs[t,]<-rmvnorm(1,

...
x<-matrix(NA,nrow=iter_max+100,ncol=4) #position and velocity
colnames(x)<-c("pos_horiz","pos_vert","velo_horiz","velo_vert")
y<-matrix(NA,nrow=iter_max+100,ncol=2) #observed position
...
A2<-diag(c(1,1,0,0))
B2<-c(200,50,0,0)
for(t in (iter_max+1):(iter_max+100)){
  x[t,]<-rmvnorm(1,x[(t-1),]+B2,Sigma)
  y[t,]<-rmvnorm(1,drop(C%*%x[t,]),R)
}
plot(x[,1],x[,2],type="l")
points(y[,1],y[,2])
...
### Sampling Trajectory with Known Dynamical Parameters
...
```{r}
pars<-list()
pars[[1]]<-list("A"=A,"B"=B,"Sigma"=Sigma) #concave parabola
pars[[2]]<-list("A"=A2,"B"=B2,"Sigma"=Sigma) #linear motion, slight increase
z<-c(rep(1,iter_max),rep(2,100))
...
ggplot(data=xs,aes(x=pos_horiz,y=pos_vert))+geom_path(aes(group=id),colour="grey")+geom_path(data=x_true,size=1.3,aes(colour=factor(z)))+geom_point(data=y_obs,aes(colour=factor(z)))+theme_bw()
```
...
#save(pars,x_true,z,x_0,file="will_dat.RData")
```

### Sampling Trajectory with Unknown Dynamical Parameters
...
pars[[2]]<-list("A"=A2,"B"=B2,"Sigma"=Sigma) #linear motion, slight increase
K<-length(pars)
#split data into chunks based on z

...
#set hyperparameters
Delta0<-.75*diag(rep(lowess_resid_var,2))
hp<-list(nu0=D+4,kappa0=.01,K_A=.01*diag(D),Delta0=Delta0)
...
for(j in 1:1000){
  ...
  if(j%%100==0){
    ...
    show(ggbase+geom_path(data=xhat_plt,colour="grey",aes(group="id")))
  }
...
#lambda_hat[["R"]]<-avg_Sigma[1:ydim,1:ydim] #need so sample from posterior
```

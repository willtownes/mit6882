...
$$B = \begin{pmatrix} \frac{1}{2}k^2 Ia\\ kIa$$
Hence
$$(x_t|x_{t-1})\sim\mathcal{N}\left(Ax_{t-1}+B,\mathbf{0}\right)$$
(ie, the motion is deterministic so there is zero variance)
which is a linear dynamical system. Assume we observe only a noisy version of the position, $y(t)$. Hence, the observation model is
...
The curve indicates the true (discretized) trajectory, the circles represent the noisy observations. The task now is to try to infer the curve given only the circles.

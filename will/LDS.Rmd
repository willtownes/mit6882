...
hp<-list(kappa0=1,S0_df=8,R0_df=6,K_A=1*diag(D))
...
ggplot(data=xhat_plt,aes(x=pos_horiz,y=pos_vert))+geom_path(aes(group=id),colour="grey")+geom_path(data=x_true,aes(colour=factor(z)),size=1.2)+geom_point(data=y_obs,size=1.1)+theme_bw()+xlab("Horizontal Position")+ylab("Vertical Position")+ggtitle("Switching LDS with Unknown Dynamical Parameters")
```

...
hp<-list(kappa0=100,S0_df=100,R0_df=100,K_A=100*diag(D))
...
lambda_hat[["R"]]<-R[1:D,1:D]

# sample_lds_transition_params<-function(y,z,x,
...
for(j in 1:500){
  ...
  if(j%%50==0){
    ...
    xhat_plt<-rLDS_melt(list(xhat),colnames(x)[1:D])
  ...
  for(k in 1:K){
    ...
    theta_hat[k]<-bayes_mlinreg_post(t(psi),t(psibar),hyper=hp,nBurn=10,nSample=1)
  }
}
```
